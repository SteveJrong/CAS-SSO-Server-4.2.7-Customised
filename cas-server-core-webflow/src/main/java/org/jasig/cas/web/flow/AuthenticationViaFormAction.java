...
import org.jasig.cas.web.flow.exception.BadAuthcodeAuthenticationException;
import org.jasig.cas.web.flow.exception.NullAuthcodeAuthenticationException;
 ...
 * 
 * @author Steve Jrong
 * create date: 2018/1/25 5:51:07 PM
 */
...
public class AuthenticationViaFormAction {
	...
	/**
	 * Flow scope attribute that determines if authn is happening at a public
	 * workstation.
	 */
	 ...
	 * @param context
	 *            the context
	 * @return true, if both service and tgt are found, and the request is not
	 *         asking to renew.
	 * @since 4.1.0
	 */
	...
	/**
	 * Grant service ticket for the given credential based on the service and
	 * tgt that are found in the request context.
	 *
	 * @param context
	 *            the context
	 * @param credential
	...
	/**
	 * Create ticket granting ticket for the given credentials. Adds all
	 * warnings into the message context.
	 *
	 * @param context
	 *            the context
	 * @param credential
	...
	private static void putPublicWorkstationToFlowIfRequestParameterPresent(final RequestContext context) {
		if (StringUtils
				.isNotBlank(context.getExternalContext().getRequestParameterMap().get(PUBLIC_WORKSTATION_ATTRIBUTE))) {
